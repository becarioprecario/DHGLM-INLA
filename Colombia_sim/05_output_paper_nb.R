############################### Summary of results 
############################### Negative binomial model (Colombia data)
...
traplot(coda_colnb, parms=c("beta","gamma"))
dev.new(height = 6, width = 9)
denplot(coda_colnb, parms=c("beta","gamma"))

# Show estimated parameters (Mean, SD and 95% Credible Interval)
summ.mcmc <- cbind(summary(coda_colnb)$statistics[c(1:4),c(1,2)],summary(coda_colnb)$quantiles[c(1:4),c(1,5)])
