############################### Summary of results 
############################### Gaussian model with fixed effects for the subjects (Sleepstudy data)
...
beta1_est <- as.vector(estimations[[1]])
beta2_est <- as.vector(estimations[[2]])
gamma1_est <- as.vector(estimations[[3]])
precu_est <- as.vector(estimations[[4]])


summ <- rbind(beta1_est[c(1,2,3,7)],beta2_est[c(1,2,3,7)], gamma1_est[c(1,2,3,7)], precu_est[c(1,2,3,7)])
summ.is <- apply(unname(summ),c(1,2), FUN=as.numeric)
rownames(summ.is) <- c("beta0", "beta1", "gamma", "precu")
